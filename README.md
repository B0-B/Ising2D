...
<script>
    ...
    out=ce('output');l.innerHTML=xn;w.appendChild(fd);if(x=='T'){In.min="1e-2";
    In.step="1e-2";In.max="50"}else{In.min="1e-4";In.step="1e-4";In.max="10"}
    ...
    .forEach(e=>{fd.appendChild(e)})});sim()//https://github.com/B0-B/
</script>
```
